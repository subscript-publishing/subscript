...
fileprivate let DARK_LINE_COLOR: UIColor = #colorLiteral(red: 0.3012802601, green: 0.3012802601, blue: 0.3012802601, alpha: 1)
fileprivate let LIGHT_LINE_COLOR: UIColor = #colorLiteral(red: 0.6000000238, green: 0.6000000238, blue: 0.6000000238, alpha: 1)
...
fileprivate class DrawingGestureRecognizer: UIGestureRecognizer {
    ...
    func finalizeActiveStroke() {
        if runtimeModel.currentToolType.isPen && !drawingPaper.active.isEmpty {
            ...
            case .foreground: drawingPaper.foregroundStrokes.append(drawingPaper.active.simplify())
            case .background: drawingPaper.backgroundStrokes.append(drawingPaper.active.simplify())
            }
        }
        clearActiveStroke()
    }
...
fileprivate func renderPenStroke(
    ...
    context.saveGState()
    context.beginPath()
    context.setFillColor(displayMode == .dark ? stroke.options.darkUIColorSchemeColor.cgColor : stroke.options.lightUIColorSchemeColor.cgColor)
    context.addLines(between: points)
    context.closePath()
    if isHighlighted {
        let overrideColor = displayMode == .dark
            ? stroke.options.darkUIColorSchemeColor.color.withAlphaComponent(0.3).cgColor
            : stroke.options.lightUIColorSchemeColor.color.withAlphaComponent(0.3).cgColor
...
fileprivate func renderSelectionStroke(
    ...
    if let boundingBox = stroke.boundingBox(maxWidth: size.width) {
        ...
        context.setStrokeColor(
            displayMode == .dark
                ? stroke.options.darkUIColorSchemeColor.cgColor
                : stroke.options.lightUIColorSchemeColor.cgColor
        )
    ...
    fileprivate class DrawingView: UIView {
        ...
        static func computeCGImage(
            ...
            autoreleasepool {
                ...
                context.saveGState()
                context.setFillColor(CANAVS_PAPER_DARK_BG_COLOR.cgColor)
