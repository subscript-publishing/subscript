...
pub fn compile_template_file<T: Into<PathBuf>>(
    ...
    let html = ast::Node::parse_str(source);
    let f = |env: ast::NodeScope, node: ast::Node| -> ast::Node {
        match node {
            ast::Node::Element(elem) if elem.has_tag("link") => {
                ...
                ast::Node::Element(elem)
            }
            x => x,
        }
    };
    let html = html.transform(ast::NodeScope::default(), Rc::new(f));
    // println!("{:#?}", html);
}
