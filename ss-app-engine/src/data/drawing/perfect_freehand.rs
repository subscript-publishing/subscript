...
fn stride_float(from: f32, to: f32, by: f32) -> Vec<f32> {
    let mut i = from;
    let mut xs = Vec::new();
    while i <= to {
        xs.push(i);
...
//        if (nextAngle < 0.20)
        if (nextDpr < 0.0) {
            ...
            for t in stride_float(0.0, 1.0 + step, step) {
                tl = rotAround(sub(points[i].point, offset), points[i].point, std::f32::consts::PI * t);
                leftPts.push(tl);
                
                tr = rotAround(add(points[i].point, offset), points[i].point, std::f32::consts::PI * -t);
                rightPts.push(tr);
            }
    ...
    if (points.len() == 1) {
        if (!(cbool(Some(options.start.taper)) || cbool(Some(options.end.taper))) || options.last) {
            ...
            for t in stride_float(step, 1.0 + step, step) {
                dotPts.push(rotAround(start, firstPoint, std::f32::consts::PI * 2.0 * t));
            }
            return dotPts
        }
    } else {
        ...
        if (cbool(Some(options.start.taper)) || (cbool(Some(options.end.taper)) && points.len() == 1)) {
            ...
            for t in stride_float(step, 1.1, step) {
                let pt = rotAround(rightPts[0], firstPoint, std::f32::consts::PI * t);
                startCap.push(pt);
            }
        } else {
        ...
        if (cbool(Some(options.end.taper)) || (cbool(Some(options.start.taper)) && points.len() == 1)) {
            ...
            for t in stride_float(step, 1.0 + step, step) {
                endCap.push(rotAround(start, lastPoint, std::f32::consts::PI * 3.0 * t));
            }
        } else {
...
impl super::RecordedStroke {
    pub fn vector_outline_points(&self, stroke_style: super::StrokeStyle) -> Option<PointVec> {
        ...
        let outline_points = getStrokeOutlinePoints(getStrokePoints(points, &options), &options)
            .into_iter()
            .map(|point| {
                super::Point{x: point.x as f32, y: point.y as f32}
            })
            .collect::<Vec<_>>();
        Some(PointVec{points: outline_points})
    }
}
...
impl<'a> super::PointVecRef<'a> {
    pub fn vector_outline_points(&self, stroke_style: super::StrokeStyle) -> Option<PointVec> {
        ...
        let points = self.points
            ...
            .map(|point| {
                SamplePoint {
                    point: CGPoint {x: point.x as f32, y: point.y as f32},
                    pressure: 0.5,
                }
            })
            .collect::<Vec<_>>();
