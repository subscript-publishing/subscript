...
impl GetStrokeOptions {
    fn init_from_stroke(stroke: stroke::Stroke) -> Self {
        GetStrokeOptions {
            ...
            easing: stroke.options.easing,
            simulatePressure: stroke.options.simulate_pressure,
            start: stroke.options.start,
            end: stroke.options.end,
            last: false,
        }
    }
    fn new(
        ...
        smoothing: CGFloat,
        streamline: CGFloat,
        easing: super::format::stroke::Easing,
        simulatePressure: Bool,
        start: super::format::stroke::StartCap,
        end: super::format::stroke::EndCap,
    ) -> Self {
        GetStrokeOptions {
            size: size,
            thinning: thinning,
...
fn getStroke(points: Array<SamplePoint>, options: GetStrokeOptions) -> Array<CGPoint> {
    return getStrokeOutlinePoints(getStrokePoints(points, options.clone()), options)
}



/// This is the rate of change for simulated pressure. It could be an option.
const RATE_OF_PRESSURE_CHANGE: CGFloat = 0.275;
...
pub fn vector_outline_points(stroke: stroke::Stroke) -> Array<[f64; 2]> {
    if stroke.samples.len() < 3 {
        return Vec::new()
    }
    let points = stroke.clone()
        .samples
        .iter()
        .map(|sample| {
            SamplePoint {
        ...
        .collect::<Vec<_>>();
    let options = GetStrokeOptions::init_from_stroke(stroke);
    let outlinePoints = getStroke(points, options);
    outlinePoints
        .into_iter()
        .map(|point| {
...
pub fn dev() {
    // let step: CGFloat = 1.0 / 13.0;
    // println!("{:#?}", stride_float(0.0, 1.0 + step, step));
    // println!("{:#?}", stride(1, 10, 1));
}
