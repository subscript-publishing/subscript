...
impl DrawResult {
    pub fn merge(self, other: DrawResult) -> DrawResult {
        use DrawResult::*;
        match (self, other) {
            (SSMetalDrawResultNoOP, SSMetalDrawResultNoOP) => SSMetalDrawResultNoOP,
            (SSMetalDrawResultSuccess, SSMetalDrawResultSuccess) => SSMetalDrawResultSuccess,
            (SSMetalDrawResultNoOP, SSMetalDrawResultSuccess) => SSMetalDrawResultSuccess,
            (SSMetalDrawResultSuccess, SSMetalDrawResultNoOP) => SSMetalDrawResultSuccess,
            (SSMetalDrawResultErr, _) => SSMetalDrawResultErr,
            (_, SSMetalDrawResultErr) => SSMetalDrawResultErr,
        }
    }
    pub fn is_ok(&self) -> bool {
        match self {
            Self::SSMetalDrawResultNoOP => true,
            Self::SSMetalDrawResultSuccess => true,
            _ => false,
        }
    }
}
...
pub extern fn ss1_metal_view_draw_flush_and_submit(
    ...
    let f = |metal_backend_context: &mut MetalBackendContext, canvas_runtime_context: &mut CanvasRuntimeContext| {
        ...
        let result = canvas_runtime_context
            ...
            .unwrap_or(DrawResult::SSMetalDrawResultNoOP);
        if !result.is_ok() {
