    ...
    static let docs: Array<Section> = [
        ...
        Section(title: "Shaping Paramaters", overview: "These settings alter the generation of a given stroke’s vector outline points, the simplest of such being the stroke diameter (i.e. size).", [
            Item(SliderControl(
                ...
                defaultValue: {
                    return "\(SS1.Pen.PenStyle.defaultSize)"
                },
                range: {
                    let start = SS1.Pen.PenStyle.minSize
                    let end = SS1.Pen.PenStyle.maxSize
            ...
            Item(SliderControl(
                title: "Thinning",
                defaultValue: {
                    return "\(SS1.Pen.PenStyle.defaultThinning)"
                },
                range: {
                    let start = SS1.Pen.PenStyle.minThinning
                    let end = SS1.Pen.PenStyle.maxThinning
                    return "from \(start) to \(end)"
                },
            ...
            Item(SliderControl(
                title: "Smoothing",
                defaultValue: {
                    return "\(SS1.Pen.PenStyle.defaultSmoothing)"
                },
                range: {
                    let start = SS1.Pen.PenStyle.minSmoothing
                    let end = SS1.Pen.PenStyle.maxSmoothing
                    return "from \(start) to \(end)"
                },
            ...
            Item(SliderControl(
                ...
                range: {
                    let start = SS1.Pen.PenStyle.minStreamline
                    let end = SS1.Pen.PenStyle.maxStreamline
                    return "from \(start) to \(end)"
                },
