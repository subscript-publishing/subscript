...
fileprivate func lrp(_ A: SamplePoint, _ B: SamplePoint, _ t: CGFloat) -> SamplePoint {
    ...
    return SamplePoint(point: newPoint, pressure: (A.pressure + B.pressure) / 2)
}
...
fileprivate struct GetStrokeOptions {
    ...
    var easing: SS1.Stroke.Options.Easing
    /// Whether to simulate pressure based on velocity.
    var simulatePressure: Bool
    /// Cap, taper and easing for the start of the line.
    var start: SS1.Stroke.Options.StartCap
    /// Cap, taper and easing for the end of the line.
    var end: SS1.Stroke.Options.EndCap
    /// Whether to handle the points as a completed stroke.
    let last: Bool = false
    
    init(fromStroke stroke: SS1.Stroke) {
...
fileprivate func getStrokePoints(_ points: Array<SamplePoint>, _ options: GetStrokeOptions) -> Array<StrokePoint> {
    ...
    if (pts.count == 1) {
        ...
        pts.append(add(pts[0], SamplePoint(
