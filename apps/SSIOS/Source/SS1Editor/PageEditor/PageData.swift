...
extension SS1 {
    class PageDataModel: ObservableObject, Codable {
        ...
        static func load(path: URL) -> Optional<PageDataModel> {
            ...
            if let data = data {
                if SS1.DEBUG_MODE {
                    print("[INFO] SS1.PageDataModel.load : Data loaded")
                }
                let result = try? decoder.decode(PageDataModel.self, from: data)
                if result == nil && SS1.DEBUG_MODE {
                    print("[ERROR] SS1.PageDataModel.load decider failed")
                }
                return result
            } else {
                if SS1.DEBUG_MODE {
                    print("[ERROR] SS1.PageDataModel.load : Data failed to load contents of file")
                }
                return .none
            }
        }
