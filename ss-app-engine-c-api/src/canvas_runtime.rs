...
use super::toolbar_runtime::SelectionType;
...
fn get_skia_color_for_edit_tool(
    ...
    match style {
        skia_bindings::SkPaint_Style::Fill => {
            paint.set_alpha(200);
        }
        skia_bindings::SkPaint_Style::Stroke => {
            paint.set_alpha(200);
        }
        skia_bindings::SkPaint_Style::StrokeAndFill => {
            paint.set_alpha(200);
        }
    }
    paint.set_anti_alias(true);
...
impl ActiveViewState {
    ...
    pub fn draw(
        ...
        match &self.active_tool {
            ...
            ToolTypeCmd::Erase(settings) => {
                use super::toolbar_runtime::SelectionType::*;
                let paint = get_skia_color_for_edit_tool(
                    view_info,
                    settings.selection_type.paint_style(),
                    EditToolType::Eraser,
                );
                match settings.selection_type {
                    StrikeThrough => {
                        ...
                        let mut path = get_skia_path(&outline_points);
                        canvas.draw_path(&path, &paint);
                        DrawResult::Success
                    }
                    Area => {
                        if let Some(rect) = self.device_input.as_points_ref().as_skia_box() {
            ...
            ToolTypeCmd::Transform(settings) => {
                let paint = get_skia_color_for_edit_tool(
                    view_info,
                    settings.selection_type.paint_style(),
    ...
    pub fn end_stroke(&mut self) {
        ...
        match &self.active.active_tool {
            ...
            ToolTypeCmd::Transform(settings) => {
                match settings.selection_type {
                    SelectionType::Area => {
                        let tool_selection = device_input.as_points_ref();
                        for draw_cmd in self.background.iter_mut().chain(self.foreground.iter_mut()) {
                            let draw_cmd_points = draw_cmd.get_points_ref();
                            let has_overlaps = tool_selection.overlaps(&draw_cmd_points);
                            draw_cmd.highlighted = has_overlaps;
                        }
                    }
                    SelectionType::StrikeThrough => {
                        let outline_points = render_outline_points_for_edit_tool(
                        ...
                        for draw_cmd in self.background.iter_mut().chain(self.foreground.iter_mut()) {
                            let draw_cmd_points = draw_cmd.get_points_ref();
                            let has_overlaps = transform.intersects(&draw_cmd_points);
                            draw_cmd.highlighted = has_overlaps;
                        }
                    }
                }
