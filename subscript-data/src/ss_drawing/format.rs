...
pub mod stroke {
    ...
    impl Stroke {
        ...
        pub fn to_svg_path(&self) -> (String, Vec<(f64, f64)>) {
            let points = vector_outline_points(self.clone());
            let path_points = points.clone()
                .into_iter()
                .enumerate()
                .map(|(ix, [x, y])| {
                    let avg = |x: f32, y: f32| {
                    ...
                    if let Some(next) = points.get(ix + 1) {
                ...
                .collect::<Vec<_>>();
            let mut points_num = Vec::with_capacity(path_points.len());
            let mut points_str = Vec::with_capacity(path_points.len());
            for (str, num) in path_points {
                points_str.push(str);
                points_num.push(num);
            }
            let points_str = points_str.join(" ");
            ...
            let path_attr = format!("d=\"{}z\"", points_str);
            (format!("<path {} {} />", fill_attr, path_attr), points_num)
        }
    }
...
impl DrawingDataModel {
    ...
    pub fn to_svg(&self) -> String {
        ...
        let strokes = strokes
            ...
            .collect_vec()
            .join("\n");
        // let mut stroke_paths: Vec<String> = Vec::with_capacity(rendered_strokes.len());
        // let mut xs: Vec<f64> = Vec::with_capacity(rendered_strokes.len());
        // let mut ys: Vec<f64> = Vec::with_capacity(rendered_strokes.len());
        // for (str, ps) in rendered_strokes {
        //     stroke_paths.push(str);
        //     for p in ps {
        //         xs.push(p.0);
        //         ys.push(p.1);
        //     }
